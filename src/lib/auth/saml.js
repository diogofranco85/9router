...
// Lazy-load so password/OIDC auth status routes do not hard-fail when the
// optional SAML dependency is missing from node_modules.
let samlClassPromise = null;
function loadSamlClass() {
  if (!samlClassPromise) {
    samlClassPromise = import("@node-saml/node-saml").then((mod) => mod.SAML);
  }
  return samlClassPromise;
}
...
export async function createSamlInstance(settings, origin) {
  const SAML = await loadSamlClass();
